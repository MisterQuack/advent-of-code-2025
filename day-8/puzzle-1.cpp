...
const int kConnections = 1000;
const int kLargestCircuits = 3;
...
int main() {
    ...
    for (auto& entry : edges) {
        ...
        createdConnections++;
        if (createdConnections >= kConnections) break;
    }

    std::sort(circuits.begin(), circuits.end(), [](const std::vector<std::string>& a, const std::vector<std::string>& b){ return a.size() > b.size(); });
    long long answer = 1;
    for (int i = 0; i < kLargestCircuits; i++) answer *= circuits[i].size();

    std::cout << "Multiplications of the three largest circuits size: " + std::to_string(answer) << std::endl;
    return 0;
}
